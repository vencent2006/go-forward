...
import (
	...
	"go-examples/course/handwriting-web-inf/code_25/framework/cobra"
	"go-examples/course/handwriting-web-inf/code_25/framework/contract"
	"go-examples/course/handwriting-web-inf/code_25/framework/util"
	"io/ioutil"
	"net/http"
	"os"
	"os/signal"
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		if content != nil && len(content) != 0 {
			...
			if util.CheckProcessExist(pid) {
				...
				// 确认进程已经关闭，每秒检测一次，最多检测closeWait * 2 秒
				...
				// todo: 这个WriteFile不用写pid的内容吗，看起来仅仅是单纯创建文件吗
