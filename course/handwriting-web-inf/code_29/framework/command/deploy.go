...
func deployUploadAction(deployFolder string, container framework.Container, end string) error {
	...
	for _, node := range deployNodes {
		...
		for _, action := range preActions {
			...
			if err != nil {
				logger.Error(context.Background(), "session.CombinedOutput(action)", map[string]interface{}{
					"cmd":        action,
					"connection": node,
					"err":        err,
				})
		...
		for _, action := range postActions {
			...
			bts, err := session.CombinedOutput(action)
			if err != nil {
				logger.Info(context.Background(), "session CombinedOutput", map[string]interface{}{
					"cmd":        action,
					"connection": node,
					"err":        err,
					"out_raw":    string(bts),
