...
type HadeConfig struct {
	...
	folder       string                 // 文件夹
	keyDelimiter string                 // 路径的分隔符，默认为点"."
...
func (conf *HadeConfig) removeConfigFile(folder string, file string) error {
	...
	if len(s) == 2 && (s[1] == "yaml" || s[1] == "yml") {
		...
		// 删除内存中对应的key
		delete(conf.confRaws, name) // 配置文件结构 删除
		delete(conf.confMaps, name) // 配置文件的原始信息, 删除
	}
	return nil
}
...
func replace(content []byte, maps map[string]string) []byte {
	...
	for key, val := range maps {
		replaceKey := "env(" + key + ")"
		content = bytes.ReplaceAll(content, []byte(replaceKey), []byte(val))
	}
...
func NewHadeConfig(params ...interface{}) (interface{}, error) {
	...
	if err != nil {
		// todo: 这里用到了pkg的errors，且方法是WithStack
	...
	// 起一个goroutine进行监控
	// todo: 怎么关闭这个goroutine呢
	go func() {
		// todo: 为什么要单独写个recover呢, gin的recover只能控制qin内部的流程，这个流程在gin的外部
		defer func() {
			...
			case ev := <-watch.Events:
				{
					...
					// Write 写入
					// Remove 删除
					path, _ := filepath.Abs(ev.Name) // absolute representation of path
...
func (conf *HadeConfig) find(key string) interface{} {
	...
	return searchMap(conf.confMaps, strings.Split(key, conf.keyDelimiter))
}
